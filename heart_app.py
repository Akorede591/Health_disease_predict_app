...
# Note: The actual one-hot encoding for prediction is handled manually in predict_heart_disease
# using the full_encoded_columns, which covers all the individual OHE columns.
# This list is more for conceptual understanding of which *original* columns are categorical.
...
def predict_heart_disease(features_dict):
    ...
    # Ensure the column names exactly match those generated by OneHotEncoder in training.
    ...
    # matching the structure of `X_combined` (before scaling) in `train_and_save_model.py`.

    # 4. Apply scaling using the loaded scaler.
    # The scaler was fitted on `scaler_fit_columns` (which is `full_encoded_columns`).
    # We ensure input matches this by passing the DataFrame with the correct columns.
    ...
    # This is critical for column alignment for the selector.
